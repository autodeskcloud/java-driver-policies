...
public class LatencyAndInflightCountLoadBalancingPolicyTest
  ...
  private LatencyAndInflightCountLoadBalancingPolicy latencyPolicy;
  ...
  public void setup() {
    ...
    super.setup();
    latencyPolicy = (LatencyAndInflightCountLoadBalancingPolicy) policy;
  ...
  protected BasicLoadBalancingPolicy createAndInitPolicy() {
    ...
    BasicLoadBalancingPolicy policy =
        spy(
            new LatencyAndInflightCountLoadBalancingPolicy(
  ...
  public void should_put_slowest_node_at_end_when_3_local_replicas() {
    ...
    given(pool3.getInFlight()).willReturn(5);
    given(pool5.getInFlight()).willReturn(4);
    for (int i = 0; i < 100; i++) {
  ...
  public void should_put_slowest_replicas_at_end_when_4_replicas() throws InterruptedException {
    ...
    given(pool4.getInFlight()).willReturn(5);
    given(pool5.getInFlight()).willReturn(4);
    for (int i = 0; i < 100; i++) {
      latencyPolicy.onNodeSuccess(null, 100000000, null, node1, "");
    ...
    // nodes 1, 3 and 5 always first, round-robin on the rest
    // node 3 faster -> swap
    assertThat(plan1).containsExactly(node5, node4, node3, node1, node2);
